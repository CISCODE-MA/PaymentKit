...
export class EcommerceCheckoutService {
  ...
  private async updateOrderWithPayment(
    orderId: string,
    paymentId: string,
    _gateway: 'stripe' | 'paypal',
  ): Promise<void> {
    // In real app, update database
    console.log(`Order ${orderId} updated with payment ${paymentId}`);
  }

  private async updateOrderStatus(_orderId: string, status: Order['status']): Promise<Order> {
    // In real app, update database
    const order = await this.getOrder(_orderId);
    order.status = status;
    order.updatedAt = new Date();
    return order;
  }

  private async getOrder(_orderId: string): Promise<Order> {
